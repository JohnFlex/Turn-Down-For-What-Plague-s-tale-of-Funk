...
public class FieldOfView : MonoBehaviour
{
    ...
    public LayerMask obstacleMask, detectionMask, maskWhenHide;
    ...
    void FindVisibleTargets()
    {
        ...
        for (int i = 0; i < targetsInRadius.Length; i++)
        {
            ...
            if (Vector2.Angle(dirTarget, dir) < viewAngle / 2)
            {
                ...
                Debug.Log(HideBehavior.HIDE_STATUS);

                if (HideBehavior.HIDE_STATUS)
                {
                    if(!Physics2D.Raycast(transform.position, dirTarget, distanceTarget, maskWhenHide))
                    {
